...
(function () {
  ...
  function renderSubjects() {
    ...
    if (practiceFileExists(state.level)) {
      var attendu = state.level;
      setHeader(l.label, 'Chargement des leçons…', true);
      main().innerHTML = '<section class="nx-px-hero"><h2>' + esc(l.label) + '</h2><p>Chargement des leçons…</p></section>';
      loadPracticeLevel(attendu).then(function (ok) {
        if (state.level !== attendu || state.psubject || state.dsubject) return;
        if (ok) renderPracticeSubjects(); else renderSubjects();
      });
      return;
    }
  ...
  var PRACTICE_LOADING = {};
  function practiceStore() {
    window.NexoraPrimaireContenu = window.NexoraPrimaireContenu || {};
    return window.NexoraPrimaireContenu;
  }
  function practiceLevel() { var s = practiceStore(); return s[state.level] || null; }
  var PRACTICE_ABSENT = {};
  ...
  function practiceMatch(attendu, ecrit) {
    var brut = normalize(attendu).trim(), saisi = normalize(ecrit).trim();
    var a = brut.replace(/[.,;:!?]/g, '').trim();
    var b = saisi.replace(/[.,;:!?]/g, '').trim();
    if (!brut || !saisi) return false;
    if (!a) return saisi === brut || saisi.indexOf(brut) >= 0;
    if (!b) return false;
    if (a === b) return true;
