...
function mimeType(path) {
  ...
  if (/\.js$/i.test(path)) return "text/javascript; charset=utf-8";
  return "application/octet-stream";
}

function prepareWirePayload(request, plain) {
  const accepted = String(request.headers["accept-encoding"] || "").toLowerCase();
  const canGzip = plain.length >= GZIP_MIN_BYTES && /(^|[,\s])gzip([,\s]|$)/.test(accepted);
  if (!canGzip) return { body: plain, gzip: false };
  return { body: gzipSync(plain, { level: 6 }), gzip: true };
}
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const wire = prepareWirePayload(request, plain);

    response.setHeader("Content-Type", mimeType(path));
    response.setHeader("Content-Length", String(wire.body.length));
    response.setHeader("X-Nexora-Product", entry.product_code);
    response.setHeader("X-Nexora-Raw-Bytes", String(plain.length));
    if (wire.gzip) response.setHeader("Content-Encoding", "gzip");

    console.info("Nexora secure-content OK", {
      path,
      product_code: entry.product_code,
      raw_bytes: plain.length,
      sent_bytes: wire.body.length,
      gzip: wire.gzip,
      duration_ms: Date.now() - startedAt
    });

    return response.status(200).send(wire.body);
  } catch (error) {
